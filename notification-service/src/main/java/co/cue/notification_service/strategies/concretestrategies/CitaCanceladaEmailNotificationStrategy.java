...
@AllArgsConstructor
@Slf4j
public class CitaCanceladaEmailNotificationStrategy implements NotificationStrategy {

    private final EmailService emailService;

    @Override
    public void enviar(NotificationRequestDTO request) {
        log.info("Ejecutando Estrategia CITA_CANCELADA...");
        try {
            Map<String, String> data = request.getPayload();
            
            emailService.enviarNotificacionCitaCancelada(
                    data.get("correo"),
            ...
            log.error("Error al procesar estrategia CITA_CANCELADA: {}", e.getMessage(), e);
        }
    }

    @Override
    public NotificationType getTipo() {
        return NotificationType.CITA_CANCELADA;
    }
}
